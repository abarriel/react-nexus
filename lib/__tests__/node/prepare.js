...
describe('prepare', () => {
  ...
  it('prepares stateless components correctly', async () => {
    const StatelessComponent = () => null;
    StatelessComponent[preparable.$prepare] = async (props, context) => {
      context.bar = await Promise.resolve('foo');
    };
    const context = {};
    return prepare(<StatelessComponent />, context)
      .then(() => should(context).have.property('bar').which.is.exactly('foo'));
  });
  ...
  it('doesn’t fetch the same HTTPStore more than once', async () => {
    const testHttpConf = {
      protocol: 'http',
  ...
  describe('preparedApp', () => {
    ...
    it('is rendered as the original app with native elements which has null as child', () => {
      const Foo = () => <div>{'Foo'}<div>{null}</div></div>;
      const app = <Foo />;
      return prepare(app)
      .then((preparedApp) => {
        const appHtml = ReactDOMServer.renderToString(app);
